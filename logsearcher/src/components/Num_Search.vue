    ...
    <div class="form-row">
      <div class="col-md-4 mb-3">
        <label
          ...
          title="6자리 이상, 10자리 이하 입력 필요"
          ><b>발 송 일 <font color="red">*</font></b></label
        >
        <input
          type="text"
          class="form-control"
          v-on:keyup="inputDateSlash"
          v-model="logInfo.send_date"
          v-on:keyup.enter="
            logSearch(pagingProcess.currentPage, pagingProcess.boardLimit, true)
          "
          ref="dateEt"
          id="send_date"
          placeholder="ex> YYYYMMDD, YYYY-MM-DD, YYYY/MM/DD"
          minlength="4"
          maxlength="10"
          title="4자리 이상, 10자리 이하 입력 필요"
          required
        />
      </div>
      <div class="col-md-4 mb-3">
        <label
          for="validationDefault01"
          title="4자리 이상, 14자리 이하 입력 필요"
          ><b>발 신 번 호 <font color="red">*</font></b></label
        >
        <input
          type="text"
          class="form-control"
          v-on:keyup="inputNumSlash"
          v-model="logInfo.call_number"
          v-on:keyup.enter="
            logSearch(pagingProcess.currentPage, pagingProcess.boardLimit, true)
          "
          id="call_number"
          ref="callEt"
          placeholder="ex> 010xxxxyyyy, yyyy, 010-xxxx-yyyy"
          minlength="4"
          maxlength="14"
          title="4자리 이상, 14자리 이하 입력 필요"
        />
      ...
      <div class="col-md-4 mb-3">
        <label for="validationDefault01" title="4자리 이상, 14자리 이하 입력"
          ><b>수 신 번 호</b></label
        >
        <input
          type="text"
      ...
      <div class="col-md-4 mb-3 btn-group-toggle">
        ...
        <input
          ...
          v-on:click="
            logSearch(pagingProcess.currentPage, pagingProcess.boardLimit, true)
          "
          title="상기 조건으로 당일기준 6개월 이내 자료에서 Log 조회되며 결과는 최대 1만건만 표시됨"
        />
        <input
          class="btn btn-outline-danger"
          type="reset"
          v-on:click="resetLoginfo"
          style="margin-right:3px;"
        />
        <div class="btn-group" role="group">
          ...
          <div class="dropdown-menu">
            ...
            <div class="dropdown-item" @click="changeCategory('Agent')">
              Agent
            </div>
            <div class="dropdown-item" @click="changeCategory('Web')">Web</div>
            <div class="dropdown-item" @click="changeCategory('Messenger')">
              Messenger
            </div>
          </div>
        </div>
        ...
        <label
          ...
          title="검색성능 제한으로 인해 최대 1만건까지만 조회됨"
          >Search Total Count :
          <b
            ><u>{{ count }}</u></b
          >건<font size="2"> (최대 1만건만 조회됨)</font></label
        >
      </div>
      <div class="col-md-4 mb-31" style="display:none" v-show="timeShow">
        <label
          for="validationDefault01"
          title="' : ' 는 자동으로 입력되며 수동으로 입력하셔도 됩니다."
          ><b>시간범위(Start Time)</b></label
        >
        <input
          type="text"
          @keyup.enter="
          ...
          class="form-control"
          ref="stEt"
          v-on:keyup="stTimeColon"
          v-model="logInfo.st_time"
          id="st_time"
          minlength="6"
          maxlength="8"
          placeholder="ex> hhmmss, hh:mm:ss"
          title="' : ' 는 자동으로 입력되며 수동으로 입력하셔도 됩니다."
        />
      </div>
      <div class="col-md-4 mb-31" style="display:none" v-show="timeShow">
        ...
        <input
          ...
          class="form-control"
          ref="edEt"
          v-on:keyup="edTimeColon"
          v-model="logInfo.ed_time"
          id="ed_time"
          minlength="6"
          maxlength="8"
          placeholder="ex> hhmmss, hh:mm:ss"
...
export default {
  ...
  methods: {
    ...
    inputDateSlash() {
      ...
      if (
        isFinite(replaceDate) == false ||
        date.charAt(0) == "/" ||
        date.charAt(0) == "-"
      ) {
        alert("문자는 입력하실 수 없습니다.");
        this.logInfo.send_date = replaceDate.substring(
          0,
          replaceDate.length - 1
        );
        ...
        if (
          replaceDate.charAt(6) != "0" &&
          replaceDate.charAt(6) != "1" &&
          replaceDate.charAt(6) != "2" &&
          replaceDate.charAt(6) != "3"
        ) {
    ...
    logSearch(page, size, searchTF) {
      ...
      if (this.logInfo.send_date.length < 4) {
        ...
        0 < this.logInfo.st_time.length &&
        this.logInfo.st_time.length < 8
      ) {
        alert("시간 범위는 비우거나 hhmmss 모두 입력해주세요. (기본00:00:00)");
        this.$refs.stEt.focus();
      } else if (
        0 < this.logInfo.ed_time.length &&
        this.logInfo.ed_time.length < 8
      ) {
                  ...
                  .then((res2) => {
                    ...
                    if (success) {
                      ...
                      if (count == "0") {
                        ...
                        if (count.length == 4) {
                          this.count =
                            count.charAt(0) + "," + count.substring(1);
                        } else if (count.length == 5) {
                          this.count =
                            count.substring(0, 2) + "," + count.substring(2);
                        } else {
                          this.count = count;
                        }
                      }
